...
public abstract class FourierTransform
{
  ...
  FourierTransform(int ts, float sr)
  {
    ...
    this.bandWidth = (2f / this.timeSize) * (this.sampleRate / 2f);
    this.noAverages();
    this.allocateArrays();
    this.whichWindow = NONE;
  }
  ...
  {
    if (this.real.length != r.length && this.imag.length != i.length) throw new IllegalArgumentException( "This won't work" );
	else
    {
  ...
  protected void fillSpectrum()
  {
    ...
    if (this.whichAverage == LINAVG)
    {
      ...
      for (int i = 0; i < this.averages.length; i++)
      {
        ...
        for (j = 0; j < avgWidth; j++)
        {
          ...
          if (offset < this.spectrum.length) avg += this.spectrum[offset];
		else break;
        }
        avg /= j + 1;
        this.averages[i] = avg;
    ...
    else if (this.whichAverage == LOGAVG) for (int i = 0; i < this.octaves; i++)
      {
        float lowFreq, hiFreq, freqStep;
        if (i == 0) lowFreq = 0;
		else lowFreq = (this.sampleRate / 2) / (float) Math.pow(2, this.octaves - i);
        hiFreq = (this.sampleRate / 2) / (float) Math.pow(2, this.octaves - i - 1);
        freqStep = (hiFreq - lowFreq) / this.avgPerOctave;
        float f = lowFreq;
  ...
  {
    if (numAvg > this.spectrum.length / 2) throw new IllegalArgumentException("The number of averages for this transform can be at most " + this.spectrum.length / 2 + ".");
	else this.averages = new float[numAvg];
    this.whichAverage = LINAVG;
  }
  ...
  {
    if (which < 0 || which > 1) throw new IllegalArgumentException("Invalid window type.");
	else this.whichWindow = which;
  }
  ...
  public int freqToIndex(float freq)
  {
    ...
    if (freq < this.getBandWidth() / 2) return 0;
    // special case: freq is within the bandwidth of spectrum[spectrum.length - 1]
    if (freq > this.sampleRate / 2 - this.getBandWidth() / 2) return this.spectrum.length - 1;
    // all other cases
    float fraction = freq / this.sampleRate;
    int i = Math.round(this.timeSize * fraction);
    return i;
  }
  ...
  public float indexToFreq(int i)
  {
    ...
    {
      float lastBinBeginFreq = (this.sampleRate / 2) - (bw / 2);
  ...
  public float getAverageCenterFrequency(int i)
  {
    ...
    else if ( this.whichAverage == LOGAVG )
    {
      ...
      if (octave == 0) lowFreq = 0;
	else lowFreq = (this.sampleRate / 2) / (float) Math.pow(2, this.octaves - octave);
      // and the high frequency for this octave
      hiFreq = (this.sampleRate / 2) / (float) Math.pow(2, this.octaves - octave - 1);
  ...
  {
    if ( buffer.length - startAt < this.timeSize ) throw new IllegalArgumentException( "FourierTransform.forward: not enough samples in the buffer between " + startAt + " and " + buffer.length + " to perform a transform." );
